...
function extractAuth(msg) {
  ...
  // The FIRST Received header hop is added by the sending SMTP server.
  // Gmail wraps these headers across multiple lines, so capture the whole block.
  let testedIp = '';
  const firstReceived = text.match(/^received:\s*([\s\S]*?)(?=^[a-z][a-z0-9\-]*:\s|\r\n\r\n)/im);
  if (firstReceived && firstReceived[1]) {
    const block = firstReceived[1];

    // Prefer an IP bound inside [...] on the "from" (sender) side.
    let ip = null;
  ...
  // Find IPv6 sender IP too.
  if (!testedIp) {
    const v6 = text.match(/^received:\s*from\s+[^\r\n]*\[\s*([0-9a-f:]+)\s*\]/im);
    if (v6) testedIp = v6[1];
  }

  // Fallback: X-Originating-IP
  if (!testedIp) {
    const xip = text.match(/^x-originating-ip:\s*\[?(\d{1,3}(?:\.\d{1,3}){3})\]?/im);
    if (xip) testedIp = xip[1];
  }

  // Fallback: SPF identity ip4 in Authentication-Results / Received-SPF
  if (!testedIp) {
    const spfIp = text.match(/smtp\.mailfrom[^;\r\n]*?ip(?:4):(\d{1,3}(?:\.\d{1,3}){3})/i);
    if (spfIp) testedIp = spfIp[1];
  }
